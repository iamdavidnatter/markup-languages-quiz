...
const getPairs = (q) => q.pairs || q.options || Object.entries(q.correct || {}).map(([left,right]) => ({ left, right }));
const getOrderItems = (q) => q.items || q.sequence || (Array.isArray(q.correct) ? q.correct : []);
...
function populateFilters() {
  ...
  [...new Set(QUESTION_BANK.map(q => q.topic).filter(Boolean))].sort().forEach(t => $('topic').insertAdjacentHTML('beforeend', `<option value="${escapeHtml(t)}">${escapeHtml(t)}</option>`));
  [...new Set(QUESTION_BANK.map(q => q.source).filter(Boolean))].sort().forEach(s => $('source').insertAdjacentHTML('beforeend', `<option value="${escapeHtml(s)}">${escapeHtml(s)}</option>`));
}
function startQuiz(all=false, forcedMistakes=false) {
  mode = forcedMistakes ? 'mistakes' : $('mode').value;
  let pool = [...QUESTION_BANK];
  if (mode === 'mistakes' || forcedMistakes) pool = pool.filter(q => getMistakes().includes(q.id));
  if ($('topic').value !== 'all') pool = pool.filter(q => q.topic === $('topic').value);
  if ($('source').value !== 'all') pool = pool.filter(q => q.source === $('source').value);
  pool = shuffle(pool);
...
function render() { updateStats(); renderDots(); if (session.length) renderQuestion(session[current]); }
function updateStats() {
  const answered = Object.keys(checked).length, score = session.filter(q => checked[q.id]?.correct).length;
  $('statLoaded').textContent = session.length; $('statIndex').textContent = session.length ? `${current+1} / ${session.length}` : '–'; $('statAnswered').textContent = answered; $('statScore').textContent = score; $('statPercent').textContent = answered ? Math.round(score/answered*100)+'%' : '0%';
...
function renderQuestion(q) {
  ...
  $('questionHost').innerHTML = `<article class="question-card"><div class="q-head"><div class="badges"><span class="badge">${escapeHtml(q.topic)}</span><span class="badge source">${escapeHtml(q.source)}</span><span class="badge">${escapeHtml(typeLabel(q.type))}</span></div><span class="badge">Frage ${current+1}/${session.length}</span></div><div class="q-body"><h2 class="q-title">${escapeHtml(q.question)}</h2><div id="answerBox"></div><div id="explanationBox"></div></div><div class="nav"><button id="prev" class="secondary" ${current===0?'disabled':''}>Zurück</button><div class="actions"><button id="check">Antwort prüfen</button><button id="solution" class="ghost">Lösung anzeigen</button></div><button id="next" class="secondary" ${current===session.length-1?'disabled':''}>Weiter</button></div></article>`;
  ...
  if (isChecked) showExplanation(q);
}
function renderAnswerBox(q, isChecked) {
  ...
  if (q.type === 'multiple_choice' || q.type === 'true_false') {
    ...
    const items = getOrderItems(q), arr = saved || [];
    $('answerBox').innerHTML = `<p style="color:var(--muted);">Klicke die Elemente in der richtigen Reihenfolge an.</p><div class="order-list">${items.map(item => `<button class="order-item ${arr.includes(item)?'picked':''}" data-item="${escapeHtml(item)}"><span>${escapeHtml(item)}</span><span class="order-num">${arr.includes(item)?arr.indexOf(item)+1:'+'}</span></button>`).join('')}</div><div class="actions" style="margin-top:10px"><button class="secondary" id="resetOrder">Reihenfolge zurücksetzen</button></div>`;
    ...
    const pairs = getPairs(q), options = shuffle(pairs.map(p=>p.right)); const map = saved || {};
    $('answerBox').innerHTML = pairs.map(p=>`<div class="match-row" data-left="${escapeHtml(p.left)}"><strong>${escapeHtml(p.left)}</strong><select data-left="${escapeHtml(p.left)}"><option value="">Bitte wählen</option>${options.map(o=>`<option value="${escapeHtml(o)}" ${map[p.left]===o?'selected':''}>${escapeHtml(o)}</option>`).join('')}</select></div>`).join('');
...
function checkAnswer(q, showSolution=false) {
  const correct = isCorrect(q); checked[q.id] = { correct, showSolution };
  const mistakes = getMistakes(); if (!correct) setMistakes([...mistakes, q.id]); else setMistakes(mistakes.filter(id => id !== q.id));
  markAnswers(q); showExplanation(q); updateStats(); renderDots();
}
function isCorrect(q) {
  ...
  if (q.type==='ordering') return JSON.stringify(ans||[])===JSON.stringify(getOrderItems(q));
  if (q.type==='matching') return getPairs(q).every(p => (ans||{})[p.left] === p.right);
  return false;
}
function markAnswers(q) {
  if (q.type==='multiple_choice' || q.type==='true_false') {
    ...
    const ans=answers[q.id]||[], items=getOrderItems(q); $('answerBox').querySelectorAll('.order-item').forEach(btn => { const item=btn.dataset.item; const pos=ans.indexOf(item); const correctPos=items.indexOf(item); if(pos>=0) btn.classList.add(pos===correctPos?'correct':'incorrect'); });
  } else if (q.type==='matching') {
    $('answerBox').querySelectorAll('.match-row').forEach(row => { const left=row.dataset.left; const pair=getPairs(q).find(p=>p.left===left); const val=(answers[q.id]||{})[left]; row.classList.add(val===pair?.right?'correct':'incorrect'); });
  }
}
function showExplanation(q) {
  let html = `<div class="explain"><h3>${checked[q.id]?.correct?'✅ Richtig':'❌ Nicht ganz richtig'}</h3><p>${escapeHtml(q.explanation || 'Prüfe die markierten Antworten und vergleiche sie mit der Theorie.')}</p>`;
  ...
  if (q.type==='ordering') html += `<p><strong>Richtige Reihenfolge:</strong> ${getOrderItems(q).map(escapeHtml).join(' → ')}</p>`;
  if (q.type==='matching') html += `<div class="explain-list">${getPairs(q).map(p=>`<div class="explain-item ok"><strong>${escapeHtml(p.left)}</strong> → ${escapeHtml(p.right)}</div>`).join('')}</div>`;
...
function finishQuiz() {
  ...
  const score=session.filter(q=>checked[q.id].correct).length, wrong=session.filter(q=>!checked[q.id].correct);
